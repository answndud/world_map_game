...
class StatsPageControllerTest {
	...
	void statsPageRendersPublicMetricsWithoutDashboardLinkForGuest() throws Exception {
		...
		mockMvc.perform(get("/stats"))
			...
			.andExpect(content().string(containsString("게임별 완료 수")))
			.andExpect(content().string(containsString("아케이드 상위 기록")))
			.andExpect(content().string(containsString("퀵 퀴즈 상위 기록")))
			.andExpect(content().string(containsString("CAPITAL")))
			.andExpect(content().string(containsString("FLAG")))
			.andExpect(content().string(containsString("POPULATION BATTLE")))
