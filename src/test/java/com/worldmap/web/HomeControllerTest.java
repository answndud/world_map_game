...
class HomeControllerTest {
	...
	void homePageRenders() throws Exception {
		...
		mockMvc.perform(get("/"))
			...
			.andExpect(content().string(containsString("지금 시작할 게임")))
			.andExpect(content().string(containsString("오래 버티는 게임")))
			.andExpect(content().string(containsString("짧게 푸는 게임과 추천")))
			.andExpect(content().string(containsString("기록이 이어지는 방식")))
			.andExpect(content().string(containsString("시작할 게임을 고르세요")))
			...
			.andExpect(content().string(containsString("게스트로 바로 시작한 뒤")))
