...
class RecommendationOfflinePersonaSnapshotTest {
	...
	private static final Map<String, List<String>> CURRENT_ENGINE_TOP3_SNAPSHOT = snapshot();
	...
	void currentEngineMatchesPersonaTop3Snapshot() {
		for (RecommendationPersonaBaselineScenario scenario : baselineCatalog.scenarios()) {
			...
			assertThat(actualTop3)
				...
				.containsExactlyElementsOf(CURRENT_ENGINE_TOP3_SNAPSHOT.get(scenario.id()));
		}
	}
	...
	private static Map<String, List<String>> snapshot() {
		...
		snapshot.put("P02", List.of("말레이시아", "태국", "스페인"));
		...
		snapshot.put("P08", List.of("뉴질랜드", "핀란드", "노르웨이"));
		snapshot.put("P09", List.of("아랍에미리트", "싱가포르", "덴마크"));
		snapshot.put("P10", List.of("미국", "대한민국", "영국"));
		snapshot.put("P11", List.of("캐나다", "아일랜드", "스위스"));
