...
5. 이 추천 엔진을 다음 버전에서 어떻게 개선할 것인가?

## 면접에서는 이렇게 설명하면 된다

“추천 기능은 처음부터 계산을 서버가 deterministic하게 하도록 잡았습니다. 이번 단계에서는 `RecommendationSurveyForm`으로 설문 입력을 받고, `RecommendationSurveyAnswers`라는 불변 값 객체로 바꾼 뒤, `RecommendationSurveyService`가 국가 프로필 카탈로그와 비교해 top 3를 계산합니다. 즉 추천 결과 자체는 서버가 만들고, 이후 품질 개선은 문항과 시나리오를 다듬는 방향으로 이어집니다.”

## 다음 글

다음 단계는 추천 계산 구조는 그대로 유지한 채 `후보 국가 풀을 더 넓히는 작업`이다.

즉, 바로 AI를 서비스에 붙이는 대신 먼저 “어떤 나라들을 비교 대상으로 두는가”와 “설문 품질을 어떻게 개선할 것인가”를 풍부하게 만드는 흐름으로 이어진다.
