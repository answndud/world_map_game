...
## 왜 이 단계에서 polling으로 닫았는가

이제 랭킹 기능은 아래를 모두 갖췄다.

- 게임 종료 run 저장
- Redis Sorted Set 반영
- Redis 유실 시 RDB fallback
- `/ranking` SSR 화면
- 15초 주기 자동 갱신

즉, 사용자 입장에서 “랭킹이 계속 움직인다”는 체감은 이미 나온다.

그래서 현재 MVP 단계에서는 여기서 한 번 닫는 편이 맞다.

왜냐하면 지금 바로 SSE까지 붙이면 아래 복잡도가 추가되기 때문이다.
...
반면 지금 단계에서 가장 중요한 포인트는 “왜 Redis를 썼는가”와 “왜 source of truth는 RDB인가”를 설명하는 것이다.

그래서 현재 프로젝트는 `5단계 랭킹 시스템`을 15초 polling 전략으로 마감하고, SSE/WebSocket은 `9단계 실시간성 고도화`로 넘긴다.
