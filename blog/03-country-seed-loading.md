...
flowchart TD
    ...
    B --> C{"country 테이블이 비어 있는가?"}
    C -- "No" --> D["시드 적재 생략"]
    C -- "Yes" --> E["countries.json 읽기"]
    E --> F["CountrySeedValidator 검증"]
    F --> G["Country 엔티티로 변환"]
    G --> H["CountryRepository.saveAll()"]
```
...
- 아직 운영용 마이그레이션 도구를 붙이지 않은 초기 단계에 적합하다.
...
1. 애플리케이션 시작 시 국가 17건이 적재되는가
2. 시드 초기화 로직을 다시 호출해도 중복 적재되지 않는가
...
“게임과 추천 기능이 공통으로 쓰는 기준 데이터를 먼저 `country` 테이블에 정리했습니다. 시드 파일은 JSON으로 두고, 출처와 기준 연도를 메타데이터에 함께 넣어 설명 가능하게 만들었습니다. 초기 버전에서는 국가 경계 폴리곤 대신 대표 좌표 1개만 저장했고, 현재는 수도 좌표를 사용합니다. 앱 시작 시 빈 테이블이면 시드를 적재하고, 잘못된 데이터면 검증 단계에서 바로 실패하게 해 데이터 품질 문제를 초기에 드러나게 했습니다.”
