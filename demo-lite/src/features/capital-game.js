...
function renderQuestionCard(session, round, disabled) {
  ...
  return `
    <section class="demo-hero demo-hero--compact">
      <p class="demo-eyebrow">local-state capital loop</p>
      <h1>${round.targetCountryName}</h1>
      <p class="demo-copy">
        한국어 수도 보기 4개 중 하나를 고르세요. 오답이면 같은 Stage를 다시 풀고, 정답이면 점수만 보여 준 뒤 다음 Stage로 자동 이동합니다.
      </p>
    </section>
    ...
    <section class="demo-panel">
      <div class="demo-panel-head">
        <h2>수도 보기</h2>
        <p>메인 저장소의 <code>capitalCityKr</code> 데이터만 사용합니다.</p>
      </div>
...
function renderResult(session) {
  ...
  return `
    <section class="demo-hero demo-hero--compact">
      <p class="demo-eyebrow">capital result</p>
      <h1>${title}</h1>
      <p class="demo-copy">
        총점 ${session.totalScore}점, 정답 ${session.correctAnswers}개, 총 제출 ${session.totalAttempts}회, 브라우저 최고 점수 ${session.bestScore}점입니다.
      </p>
      <div class="demo-actions">
        <button class="demo-button" type="button" data-capital-demo-action="restart">같은 모드 다시 하기</button>
