...
즉, 국가 시드가 먼저 준비되고, admin 계정이 만들어진 뒤, old local DB에 남아 있는 recommendation feedback legacy 컬럼 제약을 먼저 완화하고, legacy `game_level` 컬럼이 남아 있는 경우에만 예전 Level 2 실험 데이터와 Redis `l2` 키를 정리한 뒤 demo 데이터가 들어간다.
...
- 국기 게임 샘플 run도 아직 기본 seed에 넣지 않는다.
  - 그래서 `/stats`와 `/ranking`의 flag 보드도 첫 플레이 전까지 비어 있을 수 있다.
...
3. country seed -> admin bootstrap -> legacy schema 완화 -> legacy Level 2 data purge(필요 시) -> demo bootstrap이 다시 실행된다.
...
반대로 예전 Level 2 실험 흔적은 legacy 컬럼이 남아 있는 환경에서만 startup마다 다시 비운다.

예전 local PostgreSQL을 오래 재사용한 경우 `recommendation_feedback`에 더 이상 쓰지 않는 legacy 컬럼이 남아 있을 수 있다.
현재는 startup initializer가 아래 컬럼이 존재하면 `NOT NULL` 제약만 먼저 풀어, current demo bootstrap이 그대로 진행되게 한다.
