...
즉, 국가 시드가 먼저 준비되고, admin 계정이 만들어진 뒤, old local DB에 남아 있는 recommendation feedback legacy 컬럼 제약을 먼저 완화하고, 예전 Level 2 실험 데이터와 Redis `l2` 키를 정리한 뒤 demo 데이터가 들어간다.
...
3. country seed -> admin bootstrap -> legacy schema 완화 -> Level 2 data purge -> demo bootstrap이 다시 실행된다.
...
반대로 예전 Level 2 실험 흔적은 중복 유지하지 않고, startup마다 다시 비운다.
