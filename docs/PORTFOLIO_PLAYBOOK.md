...
- 위치 게임오버 모달에 `탈락 Stage / 클리어 / 총점` recap 3칸을 추가하고, 결과 배너에도 `마지막 Stage / 1트 클리어 Stage / 총 시도` 빠른 요약 strip을 붙여 terminal feedback를 더 빨리 읽게 정리
...
- 결과 / 게임오버 화면 copy와 시각 연출을 더 확장할지 결정
- 모바일 드래그 감도와 지구본 상호작용을 실기기 기준으로 한 번 더 확인
...
- 인구수 게임오버 모달에 `탈락 Stage / 클리어 / 총점` recap 3칸을 추가하고, 결과 배너에도 `마지막 Stage / 1트 클리어 Stage / 총 시도` 빠른 요약 strip을 붙여 terminal feedback를 더 빨리 읽게 정리
- 플레이 HUD 상태 카드를 `Stage / 점수 / 클리어 / 하트 / 난이도` 순서로 재배치하고, 모바일에서도 2열 compact grid를 유지하도록 조정
- 결과 배너 아래에 `총점 / 클리어 / 마지막 Stage` 한 줄 요약을 추가하고, 결과 통계 카드도 모바일에서 먼저 읽어야 할 순서대로 재배열
- 정답 overlay는 `정답 / +점수 · 총점`으로, 오답 overlay는 `하트 N개 남음 · 다시 추정`으로 정리하고 HUD 문구도 `방금 고른 구간`, `다음 Stage 준비 중`처럼 더 짧은 아케이드 copy로 통일
...
- 정답/오답 시각 연출을 더 확장할지 결정
- 연속 정답 보너스, 사운드 여부 같은 아케이드 규칙 확장 여부 판단
- 현재 구간 경계가 실제 플레이 감각에 맞는지 조정
- 실기기 기준 버튼 간격, HUD 밀도, 결과 테이블 가독성 미세 조정
...
- README는 이후 한 번 더 지원 링크용 landing page 구조로 재작성한다. 이 버전에서는 `Main App vs Demo Lite`를 명시적으로 나누고, 실제 스크린샷 6장과 `How I Worked With AI` 섹션을 넣어 “무엇을 만들었는가 / 왜 검토할 가치가 있는가 / AI를 어떻게 작업 방식으로 썼는가”를 GitHub 첫 화면에서 바로 읽히게 만든다
- 이후 polish에서는 README를 영어 소개문서가 아니라 한국어 제출용 landing page 톤으로 다시 맞춘다. 스크린샷은 3열 표 대신 큰 단일 이미지 섹션으로 배치해 GitHub repo 첫 화면에서도 대표 장면이 충분히 크게 보이게 만든다
...
- 첫 구현 조각으로 [demo-lite](/Users/alex/project/worldmap/demo-lite) 별도 앱 디렉터리를 추가하고, `Vite + vanilla SPA + hash route` 기준 shell을 열었다. 이 앱은 메인 Spring Boot bean graph를 건드리지 않고 `sync-shared-assets.mjs`로 `countries.json`, `flag-assets.json`, flag SVG를 `public/generated/`로 복사한 뒤 브라우저에서 fetch해 읽는다
- `demo-lite` 첫 route map은 `#/`, `#/games/capital`, `#/games/flag`, `#/games/population-battle`, `#/recommendation`으로 고정했고, 그 위에 retained loop를 하나씩 붙이는 방향으로 갔다
- [demo-lite/src/features/capital-game.js](/Users/alex/project/worldmap/demo-lite/src/features/capital-game.js)는 `5 rounds + 3 lives + 같은 문제 재시도 + 정답 후 자동 다음 문제 + localStorage best score`를 브라우저 메모리 상태로만 처리하고, [demo-lite/tests/capital-game.test.mjs](/Users/alex/project/worldmap/demo-lite/tests/capital-game.test.mjs)로 문제 pool, distractor, 오답 재시도, 점수/종료 규칙을 Node 테스트로 고정했다
- 이어서 [demo-lite/src/features/flag-game.js](/Users/alex/project/worldmap/demo-lite/src/features/flag-game.js)를 붙여 `flag asset manifest + country seed join`, `same-continent 우선 distractor`, `같은 Stage 재시도`, `정답 후 자동 다음 문제`, `localStorage best score`를 같은 local-state 패턴으로 닫았고, [demo-lite/tests/flag-game.test.mjs](/Users/alex/project/worldmap/demo-lite/tests/flag-game.test.mjs)로 playable pool join, 오답 재시도, 점수/종료 규칙을 고정했다
- [demo-lite/src/features/population-battle-game.js](/Users/alex/project/worldmap/demo-lite/src/features/population-battle-game.js)도 붙여 `population rank gap 기반 pair`, `difficulty band`, `2-choice 선택`, `같은 Stage 재시도`, `정답 후 자동 다음 Stage`, `localStorage best score`를 local-state 패턴으로 닫았고, [demo-lite/tests/population-battle-game.test.mjs](/Users/alex/project/worldmap/demo-lite/tests/population-battle-game.test.mjs)로 pair pool, 오답 재시도, 점수/종료 규칙을 고정했다
- 마지막 retained surface로 [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js)를 붙여 첫 조각에서는 `기후 / 속도 / 비용·품질 / 영어 / 안전 / 도시-자연` 6문항 compact 설문을 local-state로 계산하게 했다
- 이어서 같은 [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js)를 `20문항 / 30국가` 구조로 확장해, feedback 저장 없이도 메인 앱 질문 breadth에 더 가까운 deterministic top 3를 브라우저 안에서 계산하게 했고 [demo-lite/tests/recommendation.test.mjs](/Users/alex/project/worldmap/demo-lite/tests/recommendation.test.mjs)로 `warm premium city`, `cold nature`, `warm foodie value` 시나리오를 고정했다
- 추가로 같은 [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js)에 `survey-v4 / engine-v20` 버전 상수를 명시하고, [demo-lite/tests/recommendation.test.mjs](/Users/alex/project/worldmap/demo-lite/tests/recommendation.test.mjs)에 메인 recommendation anchor scenario인 `USA / CAN / MYS / NZL` top1 시나리오를 그대로 옮겨 demo-lite recommendation이 현재 메인 엔진 설명과 어디까지 같은지 테스트로 잠갔다
- 마지막으로 결과 화면에는 `선호 키워드 4개 + top1 narrative + 복사 가능한 한 줄 요약`을 추가해, 브라우저 포트 recommendation도 "왜 이 나라가 1위인지"를 카드와 공유 문장으로 더 짧게 설명할 수 있게 했다
- 이어서 같은 recommendation 결과 화면에 `기후 / 생활 환경 / 생활비 감각 / 초기 적응 / 정착 기반` 5축 비교 카드를 추가해, top 3 국가를 같은 기준으로 나란히 읽을 수 있게 했다
- [demo-lite/src/lib/browser-history.js](/Users/alex/project/worldmap/demo-lite/src/lib/browser-history.js)를 추가해 capital/flag/population-battle terminal result와 recommendation top1을 localStorage 최근 기록으로 남기고, [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js) 홈이 이를 `mode별 요약 + recent play list`로 다시 읽게 했다
- 이어서 같은 [demo-lite/src/lib/browser-history.js](/Users/alex/project/worldmap/demo-lite/src/lib/browser-history.js)가 `최근 mode streak`, `최근 클리어 streak`, `copyable one-line share summary`까지 계산하고, [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js)가 홈에서 복사 버튼과 함께 이를 노출하게 했다. 즉 `demo-lite` 홈은 이제 단순 index가 아니라 browser-side read model을 모아 보여 주는 lightweight dashboard다
- free static hosting 연결을 위해 [demo-lite/.node-version](/Users/alex/project/worldmap/demo-lite/.node-version), [demo-lite/public/_headers](/Users/alex/project/worldmap/demo-lite/public/_headers), [demo-lite/tests/cloudflare-pages-config.test.mjs](/Users/alex/project/worldmap/demo-lite/tests/cloudflare-pages-config.test.mjs), [demo-lite/scripts/check-cloudflare-pages-baseline.mjs](/Users/alex/project/worldmap/demo-lite/scripts/check-cloudflare-pages-baseline.mjs), [DEPLOYMENT_RUNBOOK_DEMO_LITE_CLOUDFLARE_PAGES.md](/Users/alex/project/worldmap/docs/DEPLOYMENT_RUNBOOK_DEMO_LITE_CLOUDFLARE_PAGES.md)를 추가했다. 이 조각은 Git-connected Cloudflare Pages 기준으로 `root directory = demo-lite`, `build = npm run build`, `output = dist`, `.node-version` 기반 Node pin, `_headers` 기반 cache/security 계약, 그리고 `npm run verify:pages`로 로컬에서 다시 확인하는 baseline을 저장소에 먼저 고정한 것이다
- 실제로 Cloudflare Pages 프로젝트 `worldmap-demo-lite`를 만들고 [https://worldmap-demo-lite.pages.dev/](https://worldmap-demo-lite.pages.dev/)를 첫 공개 URL로 열었다. `wrangler whoami`, `wrangler pages project create`, `wrangler pages deploy dist --branch main` 기준으로 production URL을 살렸고, `curl`로 `/`, `/generated/data/countries.json`, `/assets/*`의 `200`과 `_headers` 기반 응답 헤더를 확인했다
...
- 이어서 [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css), [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js), [demo-lite/src/routes.js](/Users/alex/project/worldmap/demo-lite/src/routes.js)를 다시 잡아 `demo-lite` light theme를 beige/teal 계열에서 `slate text + cobalt accent + white glass surface` 방향으로 리베이스했다. 홈은 `hero + retained routes + browser recent state + mode snapshot` 위계로 재배치해 메인 제품과 같은 가족으로 보이게 만들고, feature route는 같은 셸 안에서 local-state loop만 강조하도록 정리했다
- 다음 조각으로 [demo-lite/src/features/capital-game.js](/Users/alex/project/worldmap/demo-lite/src/features/capital-game.js), [demo-lite/src/features/flag-game.js](/Users/alex/project/worldmap/demo-lite/src/features/flag-game.js), [demo-lite/src/features/population-battle-game.js](/Users/alex/project/worldmap/demo-lite/src/features/population-battle-game.js), [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js)에 route별 hero/status band를 추가해, 같은 제품 family 안에서 `수도 / 국기 / 배틀 / 추천`이 서로 다른 mode로 읽히게 했다. 공통 토큰은 유지하고, route별 accent glow와 상단 정보 구조만 달리해 free-tier sibling app의 셸 품질을 더 올렸다
- 마지막으로 [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js)와 [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)를 조정해 추천 화면의 20문항을 섹션별로 다시 묶고, `답변 완료 / 남은 질문 / 비교 대상 / 결과 형식` progress strip과 compact pill summary, rank별 결과 카드 위계를 추가했다. 즉 recommendation route도 flat한 긴 설문이 아니라 `sectioned questionnaire -> deterministic top 3` 흐름으로 읽히게 맞췄다
- 이어서 같은 `demo-lite` surface를 개발자용 상태판이 아니라 플레이어용 공개 화면으로 정리했다. [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js), [demo-lite/src/routes.js](/Users/alex/project/worldmap/demo-lite/src/routes.js), [demo-lite/src/features/capital-game.js](/Users/alex/project/worldmap/demo-lite/src/features/capital-game.js), [demo-lite/src/features/flag-game.js](/Users/alex/project/worldmap/demo-lite/src/features/flag-game.js), [demo-lite/src/features/population-battle-game.js](/Users/alex/project/worldmap/demo-lite/src/features/population-battle-game.js), [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js), [demo-lite/src/lib/browser-history.js](/Users/alex/project/worldmap/demo-lite/src/lib/browser-history.js), [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 `local-state`, `manifest`, `generated`, `feedback 저장`, `deterministic`, `Stage`, `브라우저 최고 점수` 같은 내부 용어를 화면에서 걷어내고 `문제`, `최고 점수`, `다시 하기`, `추천 3곳` 같은 플레이어 언어로 통일했다. 홈의 설명성 패널도 줄이고, 실제 공개 URL screenshot을 before 기준으로, 로컬 static build screenshot을 after 기준으로 다시 확인해 카드 높이와 hero spacing도 같이 다듬었다
- 마지막으로 홈 hero의 [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js) `demo-actions` CTA를 제거했다. 아래 retained game card들이 이미 `수도 게임 열기`, `국기 게임 열기`, `배틀 열기`, `추천 시작하기` 버튼을 가지고 있어 홈 첫 화면에서 같은 목적의 CTA가 두 번 보였기 때문이다. 즉 홈 hero는 제품 소개와 현재 범위만 맡고, 실제 진입은 아래 game grid가 맡도록 역할을 더 분명히 나눴다
- 이어서 같은 홈의 `최근 플레이 요약`도 다시 줄였다. [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js)에서 6개 metric을 `즐긴 게임 / 최고 점수 / 최근 추천` 3개로 압축하고, [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 metric 카드의 패딩·반경·숫자 크기를 낮췄다. 즉 browser-history read model은 유지하되, 홈에서는 대시보드처럼 많이 보여 주지 않고 정말 필요한 요약만 남기도록 바꿨다
- 마지막으로 홈의 retained game card는 아래 링크 텍스트를 없애고 카드 전체를 앵커로 바꿨다. [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js)에서 `demo-card--route` 자체가 `href="#/..."`를 가지도록 바꾸고, [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 hover/focus를 카드 전체에 붙여 사용자가 카드 박스 아무 곳이나 눌러도 바로 진입하게 했다
- 이어서 헤더의 게임 목록 nav도 전부 제거했다. [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js)에서 `renderHeader()`가 브랜드만 렌더하도록 바꾸고, [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 `demo-nav`, `demo-nav-link` dead style을 걷어냈다. 즉 화면 상단은 전역 게임 목록이 아니라 단순한 브랜드 바만 맡고, 실제 이동은 홈의 game card나 화면 내부 액션이 맡도록 더 단순하게 정리했다
- 마지막으로 home/feature/result hero에 남아 있던 `demo-eyebrow` kicker도 전부 제거했다. [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js), [demo-lite/src/features/capital-game.js](/Users/alex/project/worldmap/demo-lite/src/features/capital-game.js), [demo-lite/src/features/flag-game.js](/Users/alex/project/worldmap/demo-lite/src/features/flag-game.js), [demo-lite/src/features/population-battle-game.js](/Users/alex/project/worldmap/demo-lite/src/features/population-battle-game.js), [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js)에서 상단 작은 설명 줄을 없애고, [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 dead selector를 같이 걷어냈다. 플레이어용 공개 화면에서는 이런 얇은 라벨보다 제목과 핵심 설명만 남기는 편이 더 직접적이라는 판단이다
- 이어서 홈 game card의 `subtitle` chip도 전부 제거하고, route metadata에서 불필요한 `subtitle`/`ctaLabel` 필드를 같이 걷어냈다. [demo-lite/src/routes.js](/Users/alex/project/worldmap/demo-lite/src/routes.js)에서 retained route를 `title + summary` 중심으로 단순화했고, [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js)와 [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서는 card 상단의 보조 라인을 걷어내 카드 자체를 더 짧게 만들었다. 또 [demo-lite/src/lib/browser-history.js](/Users/alex/project/worldmap/demo-lite/src/lib/browser-history.js)에 남아 있던 `수도 맞히기` title도 `수도 퀴즈`로 바꿔 recent history와 최고 기록 문구까지 같은 이름을 쓰게 맞췄다
- 모바일 viewport 기준 리뷰 뒤에는 [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js), [demo-lite/src/features/capital-game.js](/Users/alex/project/worldmap/demo-lite/src/features/capital-game.js), [demo-lite/src/features/flag-game.js](/Users/alex/project/worldmap/demo-lite/src/features/flag-game.js), [demo-lite/src/features/population-battle-game.js](/Users/alex/project/worldmap/demo-lite/src/features/population-battle-game.js), [demo-lite/src/features/recommendation.js](/Users/alex/project/worldmap/demo-lite/src/features/recommendation.js), [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)를 다시 조정해 mobile first 기준으로 `hero -> 문제/질문 -> compact status` 순서를 더 우선하게 만들었다. 게임 화면은 status strip를 문제 panel 뒤로 내려 첫 선택지가 더 빨리 보이게 했고, recommendation hero의 4칸 status는 mobile에서 2칸만 남기도록 줄였다. 또 720px 이하에서는 hero title, chip, status card 크기를 함께 낮춰 실제 390px 폰 폭에서도 첫 질문/첫 선택지가 더 빨리 보이도록 보정했다
- 마지막 마감에서는 같은 파일들을 한 번 더 줄여, 720px 이하에서 header/panel/hero padding과 title scale을 다시 낮추고 game hero의 세 번째 chip을 숨기고 home card / history / metric card 패딩도 함께 축소했다. public URL 모바일 캡처 기준으로 홈은 `게임 목록`과 `최근 플레이 요약`이 한 화면에서 더 빨리 이어지고, 수도/추천은 hero가 짧아져 첫 보기와 첫 질문이 더 빨리 보이도록 정리됐다
- 이어서 [demo-lite/src/app.js](/Users/alex/project/worldmap/demo-lite/src/app.js), [demo-lite/src/routes.js](/Users/alex/project/worldmap/demo-lite/src/routes.js), [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css), [demo-lite/index.html](/Users/alex/project/worldmap/demo-lite/index.html), [demo-lite/public/_headers](/Users/alex/project/worldmap/demo-lite/public/_headers)를 다시 묶어 `Coinbase-inspired blue / white / near-black` visual shell로 전면 리베이스했다. 홈은 `dark hero + product snapshot + white route grid` 구조로 다시 짰고, CTA는 56px pill, headline은 `Space Grotesk`, body는 `Manrope`로 분리해 public demo가 정적 체험판이 아니라 제품 landing처럼 읽히게 맞췄다. 동시에 외부 폰트를 쓴 만큼 Pages CSP도 `fonts.googleapis.com`, `fonts.gstatic.com`만 최소 허용으로 열어 로컬과 공개 URL의 시각 차이가 나지 않게 고정했다
- 후속 미세 조정으로 같은 [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 home `demo-hero`와 오른쪽 snapshot card의 near-black 계열을 더 밝은 blue gradient로 교체하고, chip/ghost/support metric의 명도 차도 함께 줄였다. 즉 공개 첫 화면은 여전히 dark section 구조를 유지하되, `검은 landing card`보다는 `차분한 blue product hero`처럼 읽히도록 대비를 한 단계 낮췄다
- 이어서 같은 [demo-lite/src/style.css](/Users/alex/project/worldmap/demo-lite/src/style.css)에서 home의 `Playable Surfaces`, `Recent Snapshot`, `Latest Runs` panel과 route/metric/history card도 pure white에 가깝던 surface를 옅은 blue tint로 다시 보정했다. hero만 푸르게 바뀌고 아래 카드가 너무 무채색으로 남으면 위아래가 분리돼 보이기 때문에, border와 background gradient를 함께 옮겨 홈 전체가 같은 family로 읽히게 맞췄다
