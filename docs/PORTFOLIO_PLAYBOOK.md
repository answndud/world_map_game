...
- `demo-lite` 다음 조각으로는 현재 dirty working tree 기준으로 먼저 열린 public URL을 저장소 commit 기준으로 다시 맞춘다. 즉 `demo-lite` 전체 변경을 커밋/푸시하고, Git-connected Pages production source와 실제 공개 URL이 같은 상태가 되도록 정리한다
