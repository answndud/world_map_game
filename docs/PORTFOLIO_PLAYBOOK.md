...
- `정확 숫자 4개 보기형`을 `빠르게 읽을 수 있는 Level 1 문제 표현`으로 재정의
- HUD 피드백과 결과 디브리프 화면의 시각 완성도 높이기
- 정답/오답 연출, 연속 정답 보너스, 사운드 여부 같은 아케이드 polish 결정
- Level 1 보기 표현을 `구간형`으로 바꿀지 `압축 수치형`으로 유지할지 확정
...
- 왜 Level 1은 정확 숫자 4개 보기보다 구간형 또는 압축 표현이 더 적합한가
...
- 왜 Level 1 표현 방식은 아직 한 번 더 다듬을 여지가 있는가
