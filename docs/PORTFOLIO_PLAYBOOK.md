...
- local demo bootstrap은 current `survey-v4 / engine-v14` 피드백 샘플 5개를 함께 만들어, fresh local 환경에서도 `/dashboard/recommendation/feedback`이 `rank drift 줄이기` 메모를 바로 보여 주도록 정리
- `/Users/alex/project/worldmap/docs/PLAYER_COPY_AND_ADMIN_SPLIT_PLAN.md`로 public copy와 admin 운영 화면 분리 설계 정리
- `/dashboard/recommendation/persona-baseline` read-only 화면으로 현재 baseline 18/18, weak scenario 자동 계산, anchor drift 7개, active-signal 4개를 운영 화면에서 바로 확인
...
- baseline은 18 / 18을 유지하고 anchor drift도 7개까지 줄였으니, 다음엔 `P07 / P08 / P10`처럼 아직 운영 우선순위에 남는 drift를 실제 만족도와 함께 다시 볼지 판단
