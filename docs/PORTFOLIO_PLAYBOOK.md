...
- `PopulationGameSession`, `PopulationGameRound` 도메인과 보기형 출제 구조 추가
- 인구수 근접 국가를 우선 섞는 `PopulationGameOptionGenerator` 구현
- `POST /api/games/population/sessions`, `GET /round`, `POST /answer`, `GET /{sessionId}` API 추가
- SSR 시작 페이지, 플레이 셸, 결과 페이지 추가
- 홈 / 시작 / 결과 화면에 공통 `cold space` 테마 1차 적용
- 옵션 생성 단위 테스트와 전체 게임 흐름 통합 테스트 통과
