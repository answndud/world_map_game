...
- 플레이 HUD에 `선택 상태 / 진행 가이드` 영역을 두어 제출 전후 다음 행동을 더 분명하게 안내
...
- 공통 CSS에서 버튼, 패널, 입력창, 모달, 테이블 셸, 배지의 모서리를 각지게 통일하고, 컷 코너 실루엣과 스타일 버전 쿼리까지 적용해 사이트 전반의 HUD 톤과 실제 반영 경로를 함께 정리
