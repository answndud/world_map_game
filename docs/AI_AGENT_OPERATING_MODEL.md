...
2. 서비스용 AI 기능 운영
   - 추천 설명 기능에 LLM을 어떤 방식으로 붙일지
   - 어디까지를 서버가 결정하고 어디까지를 모델이 설명할지
...
5. 실제 서비스의 AI 기능도 처음에는 `단일 추천 설명 서비스`로 구현한다.
...
- 추천 설명 기능의 프롬프트와 평가 방식을 안정적으로 관리한다.

트리거 예시:

- 설문 기반 추천 결과 설명 프롬프트 작성
- OpenAI / Claude 중 한 제공자와 연결
- 추천 설명 품질을 비교 평가
...
## 서비스 안에 넣을 실제 AI 기능 구조

이 프로젝트는 추천 기능 때문에 OpenAI 또는 Claude API를 붙일 수 있다.
하지만 여기서도 `멀티 에이전트`보다 `단일 설명 생성기`가 먼저다.
...
- `RecommendationExplanationService`
  - 계산된 결과를 자연어 설명으로 바꿈
- `LlmClient`
  - OpenAI / Anthropic 제공자 어댑터
- `PromptTemplateRepository`
  - 프롬프트 템플릿과 변수 관리
- `RecommendationExplanationFallback`
  - 모델 실패 시 정적 설명 제공
...
- 추천 결과 자체는 서버가 결정한다.
- 모델은 설명만 생성한다.
- 제공자를 바꿔도 도메인 로직은 그대로 유지된다.
- 프롬프트 실험과 평가를 별도로 관리할 수 있다.

## OpenAI와 Claude를 실제 서비스에 붙일 때의 선택

### OpenAI 쪽으로 갈 때

적합한 경우:

- Responses API 중심 설계를 따르고 싶을 때
- 추후 tool use, structured output, tracing 같은 에이전트형 확장을 염두에 둘 때

권장 방식:

- 처음에는 추천 설명 생성만 담당하게 둔다.
- 모델 출력은 가능한 한 구조화된 필드 또는 제한된 포맷으로 받는다.
- 비용 절감을 위해 먼저 가장 좋은 모델로 품질 기준을 잡고, 이후 더 작은 모델을 내려보며 비교한다.

### Claude 쪽으로 갈 때

적합한 경우:

- 프롬프트 실험, 템플릿/변수 관리, 콘솔 기반 개선 흐름을 더 중시할 때
- tool use와 prompt iteration을 빠르게 반복하고 싶을 때

권장 방식:

- prompt template과 variable을 먼저 고정한다.
- eval 케이스를 함께 관리한다.
- tool use가 필요 없는 한, 설명 생성에는 굳이 복잡한 툴 루프를 붙이지 않는다.

### 내 판단

이 프로젝트의 첫 버전에서는 `OpenAI든 Claude든 단일 설명 생성 API`만 붙이면 충분하다.
에이전트형 런타임 구조는 과하다.

이 판단은 공식 문서의 "start small" 원칙에서 나온 추론이다.
...
4. 서비스 런타임 AI는 `추천 설명 생성` 하나만 먼저 붙인다.
5. 멀티 에이전트는 `리서치 / 리뷰 / 병렬 문서화`에 한정한다.
