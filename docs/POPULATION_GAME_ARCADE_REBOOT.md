...
- 완료:
  ...
  - `PopulationGameOptionGenerator`로 보기 4개 생성
  - SSR 시작 / 플레이 / 결과 페이지, HUD, 게임오버 모달 1차 구현
  - 옵션 생성 단위 테스트와 상태 전이 통합 테스트 구현
- 남은 문제:
  - 보기 숫자 4개를 그대로 읽는 방식은 게임성보다 피로도가 높다.
  - HUD와 결과 화면의 아케이드 연출은 아직 위치 게임보다 단순하다.
  - Level 1을 `구간형`으로 갈지 `압축 수치형`으로 갈지 최종 확정이 남아 있다.
...
둘 중 추천 기본값은 `구간형 Level 1`, `정확 수치형 Level 2`다.

이유:

- Level 1은 반응 속도와 직관이 중요하다.
- Level 2에서 오차율 계산으로 더 깊은 난도를 만들 수 있다.
...
- `PopulationGame 리부트 2차`
  - Level 1 보기 표현을 `구간형` 또는 `압축 수치형`으로 확정
  - HUD / 결과 화면 polish
  - 연속 정답 보너스와 연출 보강
