...
- 브라우저 스모크 테스트
  - `./gradlew browserSmokeTest`로 `home`, `capital start -> play`, capital 게임오버 모달의 `Tab / Shift+Tab / Escape / restart 후 focus return`, `recommendation survey -> result`, `/ranking`, `/stats`를 실제 Chromium에서 검증
