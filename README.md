...
하나의 웹사이트에서 사용자가 게임 모드를 선택해 국가 위치 찾기, 수도 맞히기, 국가 인구수 맞추기, 인구 비교 퀵 배틀, AI 기반 나라 추천, 실시간 랭킹을 이용하는
`백엔드 중심 게임 플랫폼`을 만든다.
...
처음에는 `게임 2개 + 랭킹 + 추천 1개`를 안정적으로 만드는 것이 맞았고, 현재는 수도 맞히기와 인구 비교 퀵 배틀까지 포함한 `기본 게임 4개 + 랭킹 + 추천 1개` 범위로 확장했다.
...
- 국기 게임 1차 준비용 `FlagAssetCatalog`, `flag-assets.json`, sample SVG 12개와 manifest/file existence 검증 추가
...
- `/ranking`
  - 실시간 랭킹
- `/stats`
  - 공개 가능한 서비스 활동 수치와 오늘의 Top 3, 수도 퀴즈 / 인구 비교 퀵 배틀 하이라이트
...
현재 상태를 설명할 때 바로 참고할 문서는 아래 5개다.
...
수도 맞히기와 인구 비교 퀵 배틀 보드는 현재 local demo bootstrap에 따로 seed하지 않기 때문에, 첫 플레이 전까지 `/stats`와 `/ranking`의 `capital`, `population-battle` 보드는 비어 있을 수 있다.
