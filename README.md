...
- production-ready 후속으로 `Playwright` 기반 `browserSmokeTest` 레일을 추가해, `home` SSR shell, `capital start -> play`, `recommendation survey -> result`를 실제 headless Chromium에서 검증한다. 기본 `test` task와 분리해 브라우저 기동 비용이 평소 단위/통합 테스트 피드백을 막지 않게 했다.
