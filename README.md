...
- 현재는 8단계 7차 기준으로 `member`, `guestSessionKey`, 게임 세션 / 랭킹 레코드 ownership 필드, 닉네임 + 비밀번호 기반 회원가입 / 로그인 / 로그아웃, 로그인 직후 현재 브라우저의 guest 기록 귀속, `/mypage` 기록 허브, raw stage 기반 플레이 성향 요약, `/admin/**` 접근 제어, 환경변수 기반 bootstrap admin provisioning까지 연결했다.
...
- `/admin/recommendation/feedback`와 `/api/recommendation/feedback/summary`에서 `surveyVersion + engineVersion` 기준 평균 점수, 응답 수, 1~5점 분포를 읽어 설문 개선 기준으로 사용한다.
- `/admin/recommendation/persona-baseline`에서 18개 페르소나 baseline 중 weak scenario와 active-signal 비교 시나리오를 운영 화면으로 확인한다.
- 홈, 추천, 랭킹 public 화면은 내부 구현 용어보다 플레이어가 바로 이해할 수 있는 제품 언어로 다시 정리했고, 버전/집계/로드맵 같은 내부 정보는 `/admin`으로 분리하는 방향으로 간다.
- 현재는 `/admin` read-only 대시보드와 `/admin/recommendation/feedback` 운영 화면을 두고, 8단계에서 `ADMIN` role 세션 기반 접근 제어까지 연결했다.
...
- 현재 MVP의 실시간 전달 방식은 15초 polling으로 닫고, SSE/WebSocket은 `9단계 실시간성 고도화` 범위로 남겨뒀다.
- 현재 제공 경로:
