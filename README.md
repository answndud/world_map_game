...
- `CurrentMemberAccessService`는 세션의 `memberId`로 현재 회원 row를 다시 읽고 닉네임/role을 동기화한다. 홈의 계정 callout, `/login`·`/signup` 진입 분기, `/mypage`, 5개 게임 시작 페이지, 게임 세션 시작 API, `GameSessionAccessContextResolver`, admin interceptor/summary API는 이제 이 값을 공통 current-member source로 쓴다. 같은 HTTP request 안에서는 request attribute cache로 한 번만 해석해 stale 로그인 UI를 막으면서도 중복 DB 조회를 줄였다.
...
- [docs/DEPLOYMENT_RUNBOOK_RAILWAY.md](/Users/alex/project/worldmap/docs/DEPLOYMENT_RUNBOOK_RAILWAY.md)
...
- Railway용 [railway.toml](/Users/alex/project/worldmap/railway.toml) 추가
- 로컬 이미지 검증용 [Dockerfile.local](/Users/alex/project/worldmap/Dockerfile.local) 분리
...
- prod는 `ddl-auto=validate`로 schema를 수정하지 않고 검증만 하게 정리
- prod readiness probe에 Redis까지 포함
- `GameLevelRollbackInitializer`는 `worldmap.legacy.rollback.enabled=true`일 때만 켜지게 바꿔 local/test 호환성 조각으로 한정
- base [application.yml](/Users/alex/project/worldmap/src/main/resources/application.yml)은 더 이상 `local` 프로필을 기본값으로 강제하지 않음
...
- 첫 ECS 배포 전에 GitHub repository variables, workflow trigger, 필수 파일이 준비됐는지 한 번에 확인하는 [check_prod_deploy_preflight.py](/Users/alex/project/worldmap/scripts/check_prod_deploy_preflight.py) 추가
...
- Railway Redis 연결은 `SPRING_DATA_REDIS_URL` direct binding과 `host / port / username / password` 분기 둘 다 설명 가능하게 정리
- `docker build -f Dockerfile.local .`로 Java 25 기반 로컬 이미지 생성 확인

즉 현재 기본 배포 경로는 `railway.toml -> worldmap.jar -> application-prod.yml -> Railway domain`이다.
AWS ECS 관련 파일은 대안 경로로 남아 있고, 다음 배포 준비 코드는 Railway 프로젝트 생성과 첫 공개 URL smoke test다.

첫 배포 전에 아래 명령으로 입력 누락부터 먼저 확인할 수 있다.

```bash
python3 scripts/check_prod_deploy_preflight.py --repo answndud/world_map_game
```

결과는 `build/reports/deploy-preflight/prod-deploy-preflight.md`에 남고, 현재 비어 있는 GitHub repository variables가 무엇인지 바로 보여 준다.
...
- 브라우저 스모크 테스트
  - `./gradlew browserSmokeTest`로 `home`, `capital start -> play`, location / capital / population / population-battle / flag 게임오버 모달의 `Tab / Shift+Tab / Escape / restart 후 focus return`, `recommendation survey -> result`, `/ranking`, `/stats`를 실제 Chromium에서 검증
