...
- 현재 1차 public 서비스는 sample SVG 12개를 기준으로 국기 게임 Level 1을 운영한다.
- distractor는 같은 대륙 국가를 먼저 우선하고, 부족하면 전체 출제 가능 pool에서 보충한다.
...
- `FlagQuestionCountryPoolService`로 출제 가능 국가 pool 12개를 서버 read model로 고정
