...
- 오프라인 baseline과 snapshot은 현재 `survey-v4 / engine-v12` 기준으로 다시 고정했다.
...
- 현재 dynamic baseline은 `18 / 18`을 유지하면서 anchor drift를 `9개`까지 줄인 상태이고, 운영 우선 시나리오는 `P06`, `P07`, `P08`이다.
...
- local demo bootstrap은 현재 `survey-v4 / engine-v12` 기준 추천 만족도 샘플도 함께 만들어, fresh local 환경에서도 `/dashboard/recommendation/feedback`이 바로 `rank drift 줄이기` 메모까지 보여 주도록 한다.
